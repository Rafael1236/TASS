...
async function buildWeb(domain, expoPublicReplId) {
  console.log("Building web version with expo export...");
  ...
  await new Promise((resolve, reject) => {
    ...
    if (proc.stdout) {
      proc.stdout.on("data", (d) => {
        ...
        if (line) console.log(`[expo-web] ${line}`);
      });
    }
    if (proc.stderr) {
      proc.stderr.on("data", (d) => {
        const line = d.toString().trim();
        if (line) console.error(`[expo-web] ${line}`);
      });
    }
...
async function main() {
  console.log("Building static Expo Go + web deployment...");
  ...
  // Build web version first (standalone, no Metro needed)
  await buildWeb(domain, expoPublicReplId);

  // Then build iOS/Android bundles for Expo Go
